...
## Sync to upstream release-2026-07-02 (2026-07-10)

Rebased the fork onto `release-2026-07-02` (upstream `ec38251`; #193 landed the
whole provider fleet on `llm@v0.4.0`, #207 cascaded `model@v0.5.0`, #209 deleted
the legacy `message` `BaseMessage`/`TextMessage`/`MultiModalMessage` subsystem).
Done as a curated cherry-pick onto a fresh branch cut from the release tag
(interactive rebase unavailable in the harness); content-equivalent to a rebase,
13 clean commits vs the prior 26. Backup: tag `backup/multimodule-pre-resync-2026-07-10`.

**Fork set shrank 10 → 9 forked modules (+ `llm/gemini` re-forked = 10 again).**
- **`agent` UN-FORKS.** #186 (`ProviderResponseID` on `llm.Response` +
  `agent.ChatResponse`) is in `agent/v0.3.4`. Dropped the fork delta; `agent/`
  is now byte-identical to upstream. Consume `agent/v0.3.4` directly (drop replace).
- **Two stream-abandonment fixes DROPPED (upstreamed).** #192 landed both the
  `llm` forwarder `ctx.Done()` select (3c22af9) and the `llm/openai`
  single-error-emission (4ca27a0) in `llm@v0.4.0` / `llm/openai@v0.4.0`. NOT
  re-applied — cherry-picking would double-apply. Overtura's five drain-before-
  return call sites stay (harmless).
- **`llm/gemini` RE-FORKED** (was unforked since 2026-06-04). Carries `6b59863`
  (recursive structured-output schema converter + `responseMimeType` + the
  context-cache API `WithCachedContent`/`CreateCache`/`WithCacheTTL`); upstream
  `v0.3.2` only has shallow structured-output schema handling. New base
  `llm/gemini/v0.3.2`.
- **Two NEW anthropic schema-cap patches** folded in from `main` (8963d0f
  `relaxNullableUnions` 16-union cap; 758e1aa `factorSharedObjectDefs` $defs/$ref
  24-optional cap) — neither in upstream `v0.3.3`. Stack onto the existing
  `llm/anthropic` fork.

**Still forked (9), rebased onto new bases:** llm/v0.4.2, llm/openai/v0.4.3,
llm/anthropic/v0.3.3, llm/gemini/v0.3.2 (re-fork), model/v0.5.0, image/v0.1.2,
image/openai/v0.1.2, image/gemini/v0.1.4, image/openrouter (fork-only), batch.

**Absorbed from upstream / adapted during the rebase:**
- `llm/openai` Responses `SendMessages`/structured paths took upstream's
  `applyResponseHeaders(out, raw)` (RequestID/ResponseHeaders capture, #189/#190)
  while KEEPING our Responses options (prompt_cache_key, chaining, retention,
  service_tier, truncation, max_tool_calls) + the `usage()` cached-input/reasoning fix.
- `llm.TokenUsage.CacheCreation5m/1hTokens` re-applied on `llm/v0.4.2` (upstream
  still lacks them) — feeds `relay/producer.go` cost accounting.
- **openai-go v1 → v3:** upstream `image/openai/v0.1.2` moved to `openai-go/v3`;
  ported the fork's `edit.go` (EditImage) import v1→v3 — `ImageEditParams` API is
  field-compatible across the major bump (commit `60f3c99`).
- **SDK bumps clean:** anthropic-sdk 1.46→1.51 (cache-TTL/metadata/per-tier field
  names survived), genai, openai-go/v3 3.41.
- **`batch` consolidated** into one commit (`f00d742`): the fork's single-package
  batch replaces upstream's per-vendor sub-modules; overtura consumes
  `batch.NewVertexGCS`/`New`, `Submit`/`Collect`.

Dropped the obsolete cross-module pin commit (04f26d6) and the gpt-image add/drop
pair (926d74a+345e9f0, net-zero). Interim docs commits dropped; this section is
the consolidated record.

Local gate: all 12 modules `go build ./...` + `go test ./...` (non-live) GREEN.
