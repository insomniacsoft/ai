...
type Request struct {
	...
	Tools    []tool.BaseTool
	Texts    []string
	// OutputSchema constrains a chat request's response to JSON matching the
	// given schema (native structured output). Currently honored by the Gemini
	// batch client (wired into genai ResponseSchema + ResponseMIMEType), mirroring
	// the synchronous llm/gemini SendMessagesWithStructuredOutput. Nil = no
	// constraint. Ignored for embedding requests and by providers whose batch
	// API does not support a response schema.
