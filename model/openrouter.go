...
const (
	...
	// OpenRouter image-generation model IDs. [OVERTURA FORK] — re-homed from the
	// monolith; upstream's model module defines no OpenRouter image models.
...
// to their configurations. [OVERTURA FORK] — re-homed from the monolith;
// upstream's model module defines no OpenRouter image models. Pricing values
// are per-image USD estimates derived from OpenRouter's image_output token rate
// × tokens-per-image at each tier (verified against
// https://openrouter.ai/api/v1/models?output_modalities=image on 2026-04-28).
// All entries are image-only output (callers must send modalities: ["image"])
// and accept image_url content blocks for multi-turn editing.
//
