...
type Response struct {
	...
	// (OpenAI Responses API `response.id`). Empty for providers that don't expose
	// one. Callers feed it back as the previous-response id to chain server-side
	// conversation state (prompt-cache continuity). Threaded out through
	// agent.ChatResponse.
